...
export const useCancelBooking = () => {
  return useMutation<
    CancelBookingResponse,
    Error,
    { bookingId: string; companyId: string; payload: CancelBookingPayload }
  >({
    mutationFn: async ({ bookingId, companyId, payload }) => {
      ...
      const response = await apiClient.patch(
        `/booking/cancel-booking/${bookingId}/${companyId}`,
        payload,
      );
...
export const useCompleteBooking = () => {
  return useMutation<
    CompleteBookingResponse,
    Error,
    { bookingId: string; companyId: string }
  >({
    mutationFn: async ({ bookingId, companyId }) => {
      ...
      const response = await apiClient.patch(
        `/booking/complete-booking/${bookingId}/${companyId}`,
      );
      // API returns { success: true, data: {...} }, extract the data
      return response.data?.data || response.data;
    },
