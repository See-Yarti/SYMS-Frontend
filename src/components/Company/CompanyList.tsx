...
export default function CompaniesList() {
  ...
  const companies = useMemo(() => data?.data?.companies ?? [], [data]);
  ...
  const stats = useMemo(() => {
    ...
    const unverifiedCount = allCompanies.filter(
      (c: any) => !c.isVerified,
    ).length;
  ...
  useEffect(() => {
    if (updateCompanyId && updateCompanyData) {
      ...
      setValue(
        ...
        updateCompanyData.tradeLicenseExpiryDate
          ? new Date(updateCompanyData.tradeLicenseExpiryDate)
              .toISOString()
              .split('T')[0]
          : '',
      );
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        {company.logo ? (
                          ...
                          <div
                            className={`h-10 w-10 rounded-full flex items-center justify-center text-white text-sm font-semibold ${getAvatarColor(company.name)}`}
                          >
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              soft delete the company and cannot be undone easily.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
