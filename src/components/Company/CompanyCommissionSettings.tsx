...
function CompanyCommissionSettingsPage() {
  ...
  const { data: settingsResOld, isLoading: settingsLoadingOld, error: settingsErrorOld } = useGetCompanySettingsOld(companyId || '');
  const { data: settingsRes, isLoading: settingsLoading, error: settingsError } = useGetCompanySettings(companyId || '');

  // Check for 429 rate limit error
  const is429Error = (settingsErrorOld as any)?.response?.status === 429 || 
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        <div className="rounded-lg border border-amber-300 bg-amber-50 p-6 text-center max-w-md">
          ...
          <p className="text-amber-600 text-sm">Please wait a moment and try again.</p>
          <Button 
            variant="outline" 
            size="sm" 
            className="mt-4" 
  ...
  const fixedCommissionAmount = settingsOld?.fixedCommissionAmount != null
    ? (typeof settingsOld.fixedCommissionAmount === 'number' ? settingsOld.fixedCommissionAmount : parseFloat(settingsOld.fixedCommissionAmount) || 0)
  ...
  const lateCancelSetting = scs.LATE_CANCEL as StatusCommissionSetting | undefined;
  const noShowSetting = scs.NO_SHOW as StatusCommissionSetting | undefined;
  const customerFaultSetting = scs.CUSTOMER_FAULT as StatusCommissionSetting | undefined;
  const operatorFaultSetting = scs.OPERATOR_FAULT as StatusCommissionSetting | undefined;
    ...
    <div className="min-h-screen">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => navigate(`/companies/${companyId}/commission-settings/edit`)}
              className="gap-1.5"
            >
        ...
        <div className="rounded-lg bg-gradient-to-r from-gray-800 to-gray-900 p-6 text-white shadow-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-300">View earning structure details</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="space-y-6 lg:col-span-2">
              ...
              <CardContent className="space-y-4">
                <div>
                  <div className="text-4xl font-bold text-gray-900">
                    {isFixedMode ? `${fixedCommissionAmount} USD` : `${baseRateNum}%`}
                  </div>
                  <p className="mt-1 text-sm text-gray-600">Base rate on completed rides</p>
                </div>
              ...
              <CardContent className="flex items-center justify-between p-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-300">100% goes to platform - Zero operator split</p>
                  </div>
                </div>
                <div className="rounded-full bg-[#F56304] px-4 py-2">
                  <span className="text-2xl font-bold">{operatorFaultSetting?.penaltyPercentage || 0}%</span>
                </div>
              </CardContent>
            </Card>
            ...
            <div>
              <div className="mb-4 flex items-center gap-2">
                ...
                <h2 className="text-base font-semibold text-gray-900">Customer Penalty Structures</h2>
              </div>
                ...
                <Card className="overflow-hidden border-l-4 border-l-orange-400 shadow-sm">
                  <CardHeader className="pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <CardTitle className="text-sm font-semibold">Late Cancel</CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <div>
                      <p className="text-xs font-medium uppercase tracking-wide text-gray-600">Customer Pays</p>
                ...
                <Card className="overflow-hidden border-l-4 border-l-red-400 shadow-sm">
                  <CardHeader className="pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <CardTitle className="text-sm font-semibold">No Show</CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <div>
                ...
                <Card className="overflow-hidden border-l-4 border-l-blue-400 shadow-sm">
                  <CardHeader className="pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <CardTitle className="text-sm font-semibold">Customer Fault</CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <div>
          ...
          <div className="space-y-6">
            {/* CDW Insurance Card */}
            <Card className={`overflow-hidden shadow-sm ${cdwSettings?.cdwEnabled ? 'border-l-4 border-l-green-500' : 'border-l-4 border-l-gray-300'}`}>
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Shield className={`h-5 w-5 ${cdwSettings?.cdwEnabled ? 'text-green-600' : 'text-gray-400'}`} />
                    <CardTitle className="text-sm font-semibold">CDW Insurance</CardTitle>
                  </div>
                  {settingsLoading ? (
              ...
              <CardContent className="space-y-3">
                {!settingsRes ? (
                  ...
                  <>
                    <div>
                      <p className="text-xs font-medium uppercase tracking-wide text-gray-600">Coverage Status</p>
                      <div className="mt-1 flex items-center gap-2">
                        <span className="h-2 w-2 rounded-full bg-green-600" />
                        <span className="text-sm font-medium text-green-800">Active & Available</span>
                      </div>
                    </div>
                    <div>
                      <p className="text-xs font-medium uppercase tracking-wide text-gray-600">Operator Bracket</p>
                      <div className="mt-2 flex items-center gap-2">
                        <span className="text-sm font-semibold text-gray-900">{cdwSettings.cdwMinPercentage || '0'}%</span>
                        <div className="h-2 flex-1 rounded-full bg-green-200">
                          <div className="h-2 w-1/2 rounded-full bg-green-500" />
                        </div>
                        <span className="text-sm font-semibold text-gray-900">{cdwSettings.cdwMaxPercentage || '0'}%</span>
                      </div>
                    </div>
                    <div className="rounded-lg bg-green-50 p-3">
                      <p className="text-xs font-medium uppercase tracking-wide text-gray-600">Platform Commission</p>
                      <p className="mt-1 text-2xl font-bold text-green-700">{cdwSettings.cdwCommissionPercentage || '0'}%</p>
                    </div>
                  </>
                ) : (
                  <div className="rounded-lg bg-gray-50 p-4 text-center">
                    <p className="text-sm text-gray-600">CDW is not enabled for this company</p>
                    <p className="mt-1 text-xs text-gray-500">Enable it in edit mode to configure CDW settings</p>
                  </div>
                )}
              </CardContent>
            </Card>
            ...
            {isFixedMode && (
              <div>
                <div className="mb-4 flex items-center gap-2">
                  ...
                  <h2 className="text-base font-semibold text-gray-900">Edge Case Rules</h2>
                </div>
                <div className="space-y-3">
                  {/* OWE Rule */}
                  <Card className={`shadow-sm ${edgeCase === 'OWE' ? 'border-l-4 border-l-green-500' : 'border-l-4 border-l-gray-200'}`}>
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1">
                          <h3 className="font-semibold text-gray-900">OWE Rule</h3>
                          <p className="mt-1 text-xs text-gray-600">
                            Even if the collected penalty is lower, the system will still consider the full fixed
                            commission amount as receivable for this booking.
                          </p>
                        </div>
                        {edgeCase === 'OWE' && (
                  ...
                  {/* CAP Rule */}
                  <Card className={`shadow-sm ${edgeCase === 'CAP' ? 'border-l-4 border-l-green-500' : 'border-l-4 border-l-gray-200'}`}>
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1">
                          <h3 className="font-semibold text-gray-900">CAP Rule</h3>
                          <p className="mt-1 text-xs text-gray-600">
                            Commission is limited to the collected penalty amount, and any remaining commission is
                            ignored.
                          </p>
                        </div>
                        {edgeCase === 'CAP' ? (
                  ...
                  <Card className="border-l-4 border-l-[#F56304] shadow-sm">
                    <CardContent className="p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <h3 className="font-semibold text-gray-900">Fixed Mode</h3>
                          <p className="mt-1 text-xs text-gray-600">
                            Fixed commission directly deducted from operator earnings per completed ride
                          </p>
                        </div>
                      </div>
                    </CardContent>
              ...
              <Card className="border-l-4 border-l-[#F56304] shadow-sm">
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <h3 className="font-semibold text-gray-900">Percentage Mode</h3>
                      <p className="mt-1 text-xs text-gray-600">
                        Apply penalty first, then split commission as percentage between platform and operator
                      </p>
                    </div>
                  </div>
                </CardContent>
