...
const CarClassList = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useFetchData<CarClassAPIType[]>('car-class', ['car-classes'], {
    staleTime: 0,
    onError: (err: any) => {
  ...
  const { mutateAsync: createCarClass, isPending: isCreating } = usePostData<
    any,
    any
  >('car-class', {
    onSuccess: async () => {
  ...
  const onAddSubmit = async (data: CarClassFormValues) => {
    const postBody = {
      ...
      description:
        data.description ||
        `${data.size} ${data.body} ${data.transmission} ${data.fuel}`,
    };
  ...
  const handleToggleStatus = (slug: string, currentActive: boolean) => {
    ...
    debounceTimers.current[slug] = setTimeout(async () => {
      try {
        await apiClient.put(`car-class/active/slug/${slug}`);
        ...
        if (err?.response?.status === 429) {
          ...
          setTimeout(
            () => setToggleLock((prev) => ({ ...prev, [slug]: false })),
            2500,
          );
        } else {
  ...
  return (
    <div className="p-6 min-h-screen">
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <Switch
                          ...
                          onCheckedChange={() =>
                            handleToggleStatus(row.slug, row.isActive)
                          }
                          disabled={
                            isToggling === row.slug || !!toggleLock[row.slug]
                          }
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`inline-flex items-center px-3 py-1 text-xs font-medium rounded-full ${category.bg} ${category.text}`}
                        >
                          {category.label}
                        </span>
                      </td>
      ...
      >
        <DialogContent
          className="max-w-[518px] max-h-[518px] p-10"
          ...
          <form onSubmit={addHandleSubmit(onAddSubmit)} className="space-y-6">
            ...
            >
              <div className="space-y-1">
                ...
                <Select
                  value={size}
                  onValueChange={(val) =>
                    addSetValue('size', val, { shouldValidate: true })
                  }
                >
              ...
              <div className="space-y-1">
                ...
                <Select
                  value={body}
                  onValueChange={(val) =>
                    addSetValue('body', val, { shouldValidate: true })
                  }
                >
                  <SelectTrigger className="h-9">
                    <SelectValue placeholder="Select" />
              ...
              <div className="space-y-1">
                ...
                <Select
                  value={transmission}
                  onValueChange={(val) =>
                    addSetValue('transmission', val, { shouldValidate: true })
                  }
                >
                  <SelectTrigger className="h-9">
                    <SelectValue placeholder="Select" />
                ...
                {addErrors.transmission && (
                  <p className="text-xs text-red-500 mt-1">
                    {addErrors.transmission.message}
                  </p>
                )}
              </div>
              <div className="space-y-1">
                ...
                <Select
                  value={fuel}
                  onValueChange={(val) =>
                    addSetValue('fuel', val, { shouldValidate: true })
                  }
                >
                  <SelectTrigger className="h-9">
                    <SelectValue placeholder="Select" />
              ...
              <div className="flex flex-wrap items-center gap-2 bg-background border border-input rounded-lg px-3 py-2 min-h-[36px]">
                {size && (
                  <span className="inline-flex items-center px-2 py-1 text-xs font-medium bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-200 rounded-full">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        addSetValue('size', '', { shouldValidate: true })
                      }
                ...
                {body && (
                  <span className="inline-flex items-center px-2 py-1 text-xs font-medium bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-200 rounded-full">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        addSetValue('body', '', { shouldValidate: true })
                      }
                ...
                {transmission && (
                  <span className="inline-flex items-center px-2 py-1 text-xs font-medium bg-purple-100 dark:bg-purple-900/50 text-purple-800 dark:text-purple-200 rounded-full">
                    {TRANSMISSION_TYPES.find((t) => t.code === transmission)
                      ?.name || transmission}
                    <button
                      type="button"
                      onClick={() =>
                        addSetValue('transmission', '', {
                          shouldValidate: true,
                        })
                      }
                ...
                {fuel && (
                  <span className="inline-flex items-center px-2 py-1 text-xs font-medium bg-[#F56304]/10 dark:bg-[#F56304]/20 text-[#F56304] dark:text-[#F56304] rounded-full">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        addSetValue('fuel', '', { shouldValidate: true })
                      }
            ...
            <div className="bg-orange-50 dark:bg-orange-950/30 border border-orange-300 dark:border-orange-700 rounded-lg p-3">
              <div className="flex items-center justify-between">
                ...
                <Badge
                  variant="secondary"
                  className="font-mono text-lg bg-background border border-border px-3 py-1"
                >
