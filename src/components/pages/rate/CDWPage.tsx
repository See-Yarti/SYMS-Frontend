...
import {
  useGetLocationCDWSettings,
  useUpdateLocationCDWSettings,
  validateCDWPercentage,
...
export default function CDWPage() {
  ...
  const { data: carClassesData, isLoading: carClassesLoading } = useFetchData<
    CompanyCarClass[]
  >(
  ...
  const [scope, setScope] = useState<'WHOLE_LOCATION' | 'PER_CAR_CLASS'>(
    'WHOLE_LOCATION',
  );
  const [wholeLocationPercentage, setWholeLocationPercentage] = useState('0');
  const [carClassPercentages, setCarClassPercentages] = useState<
    Array<{ id: string; slug: string; name: string; percentage: string }>
  >([]);
  const [revenueMethod, setRevenueMethod] = useState<
    'PART_OF_RENTAL' | 'SEPARATE'
  >('PART_OF_RENTAL');
  ...
  useEffect(() => {
    ...
    const option =
      cdwData.cdwOption || cdwData.revenueCalculationMethod || 'PART_OF_RENTAL';
    ...
    setCdwLabels(
      Array.isArray(cdwData.cdwLabels) ? [...cdwData.cdwLabels] : [],
    );

    // Load per-car-class percentages if applicable
    if (cdwData.scope === 'PER_CAR_CLASS' && cdwData.carClassPercentages) {
      const percentages = cdwData.carClassPercentages.map(
        (ccp: { companyCarClassId: string; cdwPercentage: number }) => {
          const carClass = carClasses.find(
            (cc) => cc.id === ccp.companyCarClassId,
          );
          return {
        ...
        return carClasses.map((cc) => {
          ...
          return (
            existing ?? {
              ...
              slug: cc.slug,
              name: cc.name,
              percentage: wholeLocationPercentage || '0',
            }
          );
        });
      });
  ...
  const validateForm = (
    ...
    if (scope === 'WHOLE_LOCATION') {
      ...
      const validation = validateCDWPercentage(
        pct,
        adminMinPercentage,
        adminMaxPercentage,
      );
    ...
    if (scope === 'PER_CAR_CLASS') {
      for (const cc of carClassPercentages) {
        ...
        if (isNaN(pct)) {
          return {
            valid: false,
            message: `Please enter a valid percentage for ${cc.name}`,
          };
        }
        const validation = validateCDWPercentage(
          pct,
  ...
  const buildPayload = (enabled: boolean): UpdateLocationCDWFullPayload => {
    ...
    if (scope === 'WHOLE_LOCATION') {
      payload.wholeLocationPercentage =
        parseFloat(wholeLocationPercentage) || 0;
    } else {
  ...
  const handleCdwToggleChange = async (checked: boolean) => {
    ...
    const payload: UpdateLocationCDWFullPayload = checked
      ? buildPayload(true)
      : { cdwEnabled: false };
    ...
    try {
      ...
      toast.success(
        checked ? 'CDW enabled successfully' : 'CDW disabled successfully',
      );
      setHasChanges(false);
      refetchCDW();
    } catch (error: any) {
      ...
      const errorMsg =
        error?.response?.data?.message || 'Failed to update CDW status';
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const errorMsg =
        error?.response?.data?.message || 'Failed to save CDW settings';
      toast.error(errorMsg);
  ...
  const handleCancel = () => {
    if (cdwData) {
      ...
      const option =
        cdwData.cdwOption ||
        cdwData.revenueCalculationMethod ||
        'PART_OF_RENTAL';
      setRevenueMethod(option as 'PART_OF_RENTAL' | 'SEPARATE');
      setTaxEnabled(
        cdwData.taxOnCdwEnabled ?? cdwData.cdwTaxApplicable ?? false,
      );
      setTaxType(cdwData.cdwTaxType || 'PERCENTAGE');
      setTaxValue(cdwData.cdwTaxValue || '');
      setCdwLabels(
    ...
    return (
      <div className="rounded-lg border border-destructive bg-destructive/10 p-4">
        <p className="text-sm text-destructive">
          Company and Location required to manage CDW settings.
        </p>
      </div>
    );
  }
          ...
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-gray-900">
                Enable Collision Damage Waiver
              </h2>
              <p className="text-sm text-gray-500 mt-0.5">
                Add CDW protection to your rental services
              </p>
            </div>
            <Switch
              checked={cdwEnabled}
        ...
        <>
          ...
          <div className="flex items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3">
            ...
            <span className="text-sm text-blue-800">
              <strong>Allowed CDW range:</strong> {adminMinPercentage}% –{' '}
              {adminMaxPercentage}%
              <span className="text-blue-600 ml-1">
                (Enter values within this range only)
              </span>
            </span>
          </div>
            ...
            <CardContent className="p-6">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                ...
                >
                  ...
                  >
                    {scope === 'WHOLE_LOCATION' && (
                      <div className="w-2 h-2 rounded-full bg-[#F56304]" />
                    )}
                  </div>
                  For Whole Location
                </button>
              ...
              <div className="flex items-center gap-2 mb-3">
                <h4 className="text-sm font-semibold text-gray-900">
                  {scope === 'WHOLE_LOCATION'
                    ? 'Set CDW For Whole Location'
                    : 'Set CDW Percentage by Class'}
                </h4>
                ...
                <Card className="rounded-lg border border-gray-200 bg-white">
                  <CardContent className="p-4">
                    <Label className="text-xs font-medium text-[#F56304] uppercase tracking-wide">
                      Whole Location CDW ({adminMinPercentage}% –{' '}
                      {adminMaxPercentage}%)
                    </Label>
                    <div className="mt-2 flex items-center">
                      <Input
                        ...
                        onChange={(e) =>
                          setWholeLocationPercentage(e.target.value)
                        }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {carClassPercentages.length === 0 ? (
                    <p className="text-sm text-gray-500 col-span-2">
                      No car classes available for this location
                    </p>
                  ) : (
                        ...
                        <CardContent className="p-4">
                          <Label className="text-xs font-medium uppercase tracking-wide">
                            ...
                            <span className="text-gray-400 ml-1">
                              ({adminMinPercentage}% – {adminMaxPercentage}%)
                            </span>
                          </Label>
                          <div className="mt-2 flex items-center">
                            <Input
          ...
          <Card className="rounded-xl border border-gray-200 shadow-sm">
            <CardContent className="p-6">
              ...
              <p className="text-sm text-gray-500 mt-0.5 mb-4">
                Choose how CDW amount should be calculated in your revenue
              </p>
              ...
              <div className="space-y-4">
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    >
                      {revenueMethod === 'PART_OF_RENTAL' && (
                        <div className="w-2.5 h-2.5 rounded-full bg-[#F56304]" />
                      )}
                    </div>
                    <div>
                      <p className="font-medium text-gray-900">
                        Consider CDW as Part of Rental Revenue
                      </p>
                      <p className="text-sm text-gray-500 mt-0.5">
                        CDW amount will be included in the total rental revenue
                        calculation
                      </p>
                      <div className="mt-3 rounded-lg bg-gray-100 p-3">
                        <p className="text-xs text-gray-500">Example:</p>
                        <p className="text-sm text-gray-700">
                          Rental: $100 + CDW (15%): $15 ={' '}
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-medium text-gray-900">
                        Consider CDW Separate from Rental Revenue
                      </p>
                      <p className="text-sm text-gray-500 mt-0.5">
                        CDW amount will be tracked separately from rental
                        revenue
                      </p>
                      <div className="mt-3 rounded-lg bg-gray-100 p-3">
                        <p className="text-xs text-gray-500">Example:</p>
                        <p className="text-sm">
                          <span className="text-[#F56304] font-medium">
                            Rental Revenue: $100
                          </span>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-0.5">
                      Enable if tax is applicable on CDW amount
                    </p>
                  </div>
                  <Switch
                    checked={taxEnabled}
          ...
          <Card className="rounded-xl border border-gray-200 shadow-sm">
            <CardContent className="p-6">
              <h3 className="text-base font-semibold text-gray-900 mb-1">
                CDW – Offerable Coverage Items
              </h3>
              <p className="text-sm text-gray-500 mb-4">
                Select up to 5 coverage items to show in quote and booking UI
                when CDW is offered to customers.
              </p>
