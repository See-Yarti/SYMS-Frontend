...
export default function BlackoutPage() {
  ...
  const { data: blackouts = [], refetch: refetchBlackouts } = useFetchData<
    Blackout[]
  >(
    canOperate
      ? `blackout/get-by-location/${companyId}/${paramLocationId}`
      : '',
  ...
  const [editingBlackoutId, setEditingBlackoutId] = React.useState<
    string | null
  >(null);
  const { data: editingBlackout } = useFetchData<Blackout>(
    companyId && editingBlackoutId
      ? `blackout/get-single/${companyId}/${editingBlackoutId}`
      : '',
  ...
  const { mutateAsync: createBlackout } = usePostJson('blackout/add-blackout', {
    ...
    onError: (error: any) => {
      toast.error(
        error?.response?.data?.message || 'Failed to create blackout',
      );
    },
  });
  ...
  const { mutateAsync: updateBlackout } = usePatchDataBlackout({
    ...
    onError: (error: any) => {
      toast.error(
        error?.response?.data?.message || 'Failed to update blackout',
      );
    },
  });
  ...
  const { mutateAsync: deleteBlackout } = useDeleteData({
    ...
    onError: (error: any) => {
      toast.error(
        error?.response?.data?.message || 'Failed to delete blackout',
      );
    },
  });
  ...
  const [pendingDelete, setPendingDelete] = React.useState<Blackout | null>(
    null,
  );

  const filteredBlackouts = blackouts.filter(
    (blackout) =>
      blackout.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
      blackout.locations.some((loc) =>
        loc.city.toLowerCase().includes(searchQuery.toLowerCase()),
      ) ||
      blackout.carClasses.some((cc) =>
        cc.carClass.name.toLowerCase().includes(searchQuery.toLowerCase()),
      ),
  );
                  ...
                  <div className="flex flex-col items-center justify-center gap-4">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-sm text-gray-500 max-w-md">
                        {searchQuery ? (
                          <>
                            Try adjusting your search filters. We will show
                            blackout records that match your criteria as soon as
                            they are available.
                          </>
                        ) : paramLocationId ? (
                          <>
                            No blackouts found for this location. Click{' '}
                            <span className="font-semibold">Add Blackout</span>{' '}
                            to create one.
                          </>
                        ) : (
      ...
      {deleteDialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="bg-card rounded-2xl p-6 shadow-lg border max-w-sm w-full">
            ...
            <div className="mb-6">
              Are you sure you want to delete the blackout "
